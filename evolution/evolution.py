...
from dictableobject import UniqueStatusObject
...
class Evolution:
	...
	def Selection(population: Population,total_dna_bits: int,survivor_percentage: float,elite_percentage: float,mutation_chance: float,pop_capacity=None):
		...
		Args:
			...
			elite_percentage (float): Percentage of population considered to be elite and will automatically be selected to survive.
			mutation_chance (float): Percent change of an allele to mutate (switch) during crossover.
			pop_capacity (int) = Default None means capcity is same size.
		Returns:
			Population: next_gen
		Notes:
		...
		if pop_capacity == None:
			pop_capacity = len(population.census)

		while len(population_next) < pop_capacity:
			mom = -1
			dad = -1
	...
	def Breed(mom_dna: int,dad_dna: int,mutate_chance: float,total_dna_bits: int,crossover='uniform'):
		...
		if crossover == 'uniform':
			...
			raise ValueError('no other crossovers yet')
